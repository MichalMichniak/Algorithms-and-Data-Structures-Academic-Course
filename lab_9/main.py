...
def PrimMST(G:Graph_List):
    ...
    while intree[v_idx] == 0:
        ...
        tem = G.getEdge(v_idx,parent[v_idx])
        res.insertEdge(G.getVertex(v_idx),G.getVertex(parent[v_idx]), tem)
        res.insertEdge(G.getVertex(parent[v_idx]),G.getVertex(v_idx), tem)
    return res
